...
function setup() {
  ...
  $.getJSON("data/v0.3.json", function(data) {
    if (data != null) {
      for (var i = 0; i < data.length; i++) {
        ...
        shapes[i] = new TriangleFan(data[i].xLoc, data[i].yLoc, data[i].noOfVertices, data[i].rotateDir, data[i].radius);
      }
    }
  });
}
...
function keyPressed() {
  if (key == 's' || key == 'S') saveCanvas(canvas, 'v0.3', 'png');
  if (key == 'j' || key == 'J') saveDesign(JSON.stringify(shapes), "v0.3")
}
