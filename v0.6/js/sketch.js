...
function setup() {
  ...
  $.getJSON("data/v0.6.json", function(data) {
    if (data != null) {
...
function draw() {
  ...
  // Generating text on the canvas
  fill(0, 7, 6);
  textSize(200);
  textFont(titleFont1);
  text("Generative", 0, 150);
  fill(345, 80, 100);
  text("Design", 0, 350);
}
...
function keyPressed() {
  if (key == 's' || key == 'S') saveCanvas(canvas, 'v0.6', 'png');
  if (key == 'j' || key == 'J') saveDesign(JSON.stringify(shapes), "v0.6")
}
