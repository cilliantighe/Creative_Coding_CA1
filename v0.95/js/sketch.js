...
function setup() {
  ...
  $.getJSON("data/v0.95.json", function(data) {
    if (data != null) {
...
function keyPressed() {
  if (key == 's' || key == 'S') saveCanvas(canvas, 'v0.95', 'png');
  if (key == 'j' || key == 'J') {
    // The value for each shape's rotation must be reset before it's exported as a json
    for (var i = 0; i < shapes.length; i++) {
      shapes[i].unPause();
    }
    saveDesign(JSON.stringify(shapes), "v0.95");
...
function mouseDragged() {
  ...
  shapeRadius = map(constrain(mouseX, 0, width), 0, width, 10, 1000);
  ...
  var shape = new TriangleFan(mouseX, mouseY, floor(random(5, 50)), random(-0.005, 0.005), shapeRadius, shapeOpacity, random(1));
  shapes.push(shape);
}
